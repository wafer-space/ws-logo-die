			...
			if(instr_cycle == 0) begin
				...
				if(is_EXT && ext_opc == 7) begin
					...
					case(IO_addr_buff)
						...
						22: PORTC_DDR <= regs[ext_reg];
						23: PORTC <= regs[ext_reg];
					endcase
					PC <= PC + 1;
					instr_cycle <= 0;
				end
					...
					case(IO_addr_buff)
						...
						22: regs[ext_reg] <= PORTC_DDR;
						23: regs[ext_reg] <= PORTC;
						24: regs[ext_reg] <= PINC;
						25: regs[ext_reg] <= PIND;
						default: regs[ext_reg] <= 8'hFF;
					endcase
					PC <= PC + 1;
					instr_cycle <= 0;
				end
